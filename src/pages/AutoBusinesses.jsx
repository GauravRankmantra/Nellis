...
const AutoBusinesses = () => {
  ...
  const categories = [...new Set(mockBusinesses?.map(business => business.category))];
  ...
  const getCategoryIcon = (category) => {
    switch (category) {
      ...
      case 'Tire Shop': return '🛞';
      case 'Parts Store': return '🔧';
      case 'Auto Repair': return '⚙️';
      default: return '🚗';
    }
  };
  ...
  useEffect(() => {
    setLoading(true);
    axios.get('https://backend-nellis.onrender.com/api/v1/businesses')
      .then((response) => {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-blue-900 via-slate-900 to-purple-900 text-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-5xl md:text-6xl font-bold mb-6">Auto Businesses</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-12">
          <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-6">
            <div className="flex items-center space-x-3">
              ...
              <span className="font-semibold text-gray-700 text-lg">Filter by Category:</span>
            </div>
            <select
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {
            ...
            ) : (filteredBusinesses?.length > 0 ? filteredBusinesses : businesses?.data)?.map((business) => (
              <div key={business?._id} className="group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                <div className="p-8">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="text-2xl">{getCategoryIcon(business?.type)}</span>
                      <span className={`px-3 py-1 rounded-full text-sm font-semibold ${getCategoryColor(business?.type)}`}>
                        {business?.type}
                      </span>
                    </div>
                  </div>

                  <p className="text-gray-700 mb-6 leading-relaxed text-lg">{business?.description}</p>
                  ...
                  <div className="space-y-4 mb-6">
                    ...
                    <div className="flex items-center">
                      ...
                      <a href={`tel:${business?.phone}`} className="text-blue-600 hover:text-blue-800 font-medium transition-colors duration-300">
                        {business?.phone}
                      </a>
                    </div>
                    {business?.website && (
        ...
        <div className="mt-20 bg-white rounded-2xl shadow-lg p-12">
          <h2 className="text-4xl font-bold text-center mb-12 text-gray-900">Service Categories</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {categories?.map((category) => {
              const businessCount = mockBusinesses.filter(b => b.category === category).length;
              return (
                <div key={category} className="group text-center p-8 bg-gray-50 rounded-2xl hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
                  <div className="text-4xl mb-4">{getCategoryIcon(category)}</div>
                  <h3 className="text-xl font-bold mb-3 text-gray-900">{category}</h3>
                  <p className="text-gray-600 mb-4">{businessCount} business{businessCount !== 1 ? 'es' : ''}</p>
                  <button
        ...
        <div className="mt-20 bg-gradient-to-r from-blue-900 via-slate-900 to-purple-900 text-white rounded-3xl p-12">
          <div className="text-center">
            ...
            <p className="text-xl mb-8 max-w-3xl mx-auto leading-relaxed">
              Get listed in our directory and reach thousands of potential customers along Nellis Boulevard.
